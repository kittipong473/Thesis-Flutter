...
<head>
  ...
  <title>Administrator_data Page</title>
  ...
  <table class="table" style="margin-top:25px">
    ...
    <tbody>
      <?php while ($rows = mysqli_fetch_assoc($results)) { ?>
        <tr>
          <th scope="row"> <?php echo $rows["adminID"]; ?> </th>
          ...
          <?php } else if ($rows["status"] == "หัวหน้าผู้ดูแล") { ?>
            <td style="color:blue;"> <?php echo $rows["status"]; ?> </td>
          <?php } else { ?>
            <td style="color:red;"> <?php echo $rows["status"]; ?> </td>
          <?php } ?>
          <?php if ($rows["status"] == "อยู่ในระบบ") { ?>
            <td>
              <a class="btn btn-outline-danger" href="process_status_admin.php?adminID=<?php echo $rows["adminID"]; ?>">ปิดการใช้งาน</a>
            </td>
          <?php } else if ($rows["status"] == "ปิดการใช้งาน") { ?>
            <td>
                <a class="btn btn-outline-success" href="process_status_admin.php?adminID=<?php echo $rows["adminID"]; ?>">เปิดการใช้งาน</a>
            </td>
          <?php } ?>
        </tr>
      <?php } ?>
